...
int main(int argc, char *argv[]) {
    ...
    close(fd);
    sem_init(&shared_mem_ptr->sem_write, 1, num_processes);
    sem_init(&shared_mem_ptr->sem_read, 1, 0);
    ...
    for (int i = 0; i < num_processes; i++) {
        if (fork() == 0) { // Дочерний процесс
            ...
            sem_wait(&shared_mem_ptr->sem_write);
            ...
            encrypt(shared_mem_ptr->input + fragment_start,
                    ...
                    fragment_size);
            sem_post(&shared_mem_ptr->sem_read);
            exit(0);
        }
    }

    for (int i = 0; i < num_processes; i++) {
        sem_wait(&shared_mem_ptr->sem_read);
    }
    printf("Зашифрованный текст: %s\n", shared_mem_ptr->output);
    for (int i = 0; i < num_processes; i++) {
        sem_post(&shared_mem_ptr->sem_write);
    }
